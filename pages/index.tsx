...
const Home: NextPage<IProps> = ({ seo, social, countdownDate }) => {
	...
	const rendererCountdown = (props?: any) => {
		if (props?.completed) {
			...
			return (
				<div className="grid grid-flow-col gap-2 md:gap-5 text-center auto-cols-max text-gray-200 select-none">
					{['days', 'hours', 'minutes', 'seconds'].map((type, idx) => (
						<div key={idx} className="flex flex-col bg-gold/5 shadow-lg rounded-xl p-2 bg-clip-padding bg-opacity-60 border border-white/20">
	...
	return (
		<div className="">
			...
			<motion.div initial={{ top: 0, left: 0 }} animate={{ top: y - 25, left: x - 25 }} className={`hidden lg:block absolute place-self-center h-16 w-16 bg-gold rounded-full blur-2xl`} />
				...
				<Popover>
					<div className="flex lg:flex-row justify-between items-center my-8 mx-6 lg:mx-16 2xl:my-16 2xl:mx-28">
						...
						<div className="flex items-center md:hidden">
							<Popover.Button className="bg-gold rounded-md p-2 inline-flex items-center justify-center text-white">
					...
					>
						<Popover.Panel focus className="absolute z-10 top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
							<div className="rounded-lg shadow-md bg-black border border-gold/50 overflow-hidden">
								...
								<div className="block w-full px-5 py-3 bg-black border-t border-gold/50">
			...
			<main className="">
				<Element name="hero" style={{ marginTop: `${headerHeight}px` }} className="flex flex-col-reverse md:flex-row justify-center items-center md:gap-0 min-h-[90vh] w-full px-6 md:px-16 2xl:px-28">
					<div className="grid place-items-center text-center md:inline-block md:text-left w-full font-roboto mt-10 md:mt-0 lg:pr-10">
						...
						<motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="grid grid-flow-col place-items-center gap-2 bg-gold mt-8 2xl:mt-16 py-2 px-4 2xl:py-4 2xl:px-8 rounded-md">
							...
							<span className="font-semibold text-white 2xl:text-3xl">View On Opensea</span>
						</motion.button>
					</div>
					<div className="relative flex justify-center pointer-events-none">
						<div className="absolute top-0 right-0 place-self-center h-24 w-24 bg-red-900 rounded-full blur-3xl" />
						<div className="absolute bottom-0 left-0 place-self-center h-52 w-52 bg-orange-200/50 rounded-full blur-3xl opacity-50" />

						<div className="md:h-3/4 md:w-3/4 2xl:h-auto 2xl:w-auto bg-gold/5 shadow-lg rounded-xl p-6 bg-clip-padding bg-opacity-60 border border-white/20">
							<Lottie
								options={{
									loop: true,
									autoplay: true,
				...
				<div className="grid lg:grid-cols-2 items-center py-[10vh] px-6 lg:px-16 2xl:px-28">
					<div
						...
						className={`relative w-full lg:w-[80%] bg-gold/5 bg-clip-padding bg-opacity-60 border border-white/20 p-4 shadow rounded-xl overflow-hidden`}
					>
					...
					<div className="mt-14 lg:mt-0 space-y-5 2xl:space-y-8">
						...
						<p className="text-gray-300">Five rings crafted from high quality yellow gold. The genesis collection. A unique and exclusive design forged in honour of the one and only, Satoshi Nakamoto.</p>
						<p className="text-gray-300">
							Five unique rings engraved (I-VI) on one side of the ring according to the order they are crafted in. Each ring will have its own NFT minted and the generated smart code of the NFT will be
				...
				<Element name="about" className="relative grid place-items-center py-[10vh] px-6 lg:px-16 2xl:px-28">
					<div className="absolute top-0 left-0 place-self-center h-52 w-52 bg-gold/30 rounded-full blur-3xl" />
					<div className="absolute bottom-0 right-0 place-self-center h-52 w-52 bg-blue-300/50 rounded-full blur-3xl opacity-50" />

					<div className="bg-gold/5 shadow-lg rounded-xl p-6 lg:p-20 bg-clip-padding bg-opacity-60 border border-white/20">
						...
						<div className="space-y-5 text-gray-300 mt-5">
				...
				<Element name="tokenomics" className="relative py-[10vh] px-6 lg:px-16 2xl:px-28">
					<div className="absolute inset-0 bg-[url('/assets/blur-bg.jpg')] bg-cover bg-center bg-no-repeat -z-10 bg-primary-900 blur-3xl">
						<div className="h-full w-full bg-gradient-to-b from-black via-transparent to-black pointer-events-none" />
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center items-stretch gap-5">
						...
						{tokenomics.map((item, idx) => (
							<div key={idx} className="flex flex-col justify-center bg-gold/5 shadow-lg rounded-xl p-6 lg:p-10 2xl:p-16 bg-clip-padding bg-opacity-60 border border-white/20 text-center">
								<h4 className="font-roboto font-bold text-gray-100 text-xl 2xl:text-3xl uppercase">{item?.name}</h4>
				...
				<Element name="roadmap" className="max-w-[57.5rem] mx-auto pt-[10vh]">
						...
						{roadmap.map(({ position, title, subtitle, items }, idx) => (
							<div key={idx} className={`roadmap roadmap-${position}`}>
								<div className="roadmap-step">
									...
									<ul className="list list-disc list-inside text-gray-300">
			...
			{KYCModal && (
				<div className="fixed inset-0 grid place-items-center bg-black/80 select-none z-[9999]">
					<div className="absolute max-w-2xl 2xl:max-w-6xl bg-gold/5 shadow-lg rounded-2xl m-3 p-2 2xl:p-5 bg-clip-padding bg-opacity-60 border border-white/20">
			...
			<footer className="grid place-items-center gap-10 my-10 mx-6">
				<div className="flex lg:flex-row justify-between items-center lg:max-w-5xl 2xl:max-w-6xl w-full bg-gold/5 shadow-lg rounded-2xl p-5 bg-clip-padding bg-opacity-60 border border-white/20 py-10 px-6 lg:px-20">
					<div className="">
						<h4 className="font-monument font-bold text-xl 2xl:text-3xl text-gray-100 tracking-widest">Join the community</h4>
						<p className="text-lg 2xl:text-2xl text-gray-300">We are so humbled to have your early support.</p>
						<div className="flex gap-5">
							{social?.twitter && (
								<motion.button
									whileHover={{ scale: 1.05 }}
									whileTap={{ scale: 0.95 }}
									className="grid grid-flow-col place-items-center gap-2 bg-gold mt-8 py-2 px-4 rounded-md"
				...
				<p className="text-gray-100 text-center">Copyright &copy; {new Date().getFullYear()} Satoshis. All rights reserved.</p>
			</footer>
		</div>
	);
};
